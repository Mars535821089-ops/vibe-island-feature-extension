...
`X=1720`。占位启用后，紧邻占位的两个图标保留到小岛右侧，剩余图标止于小岛左边缘；
两个图标和占位组成的保护区左边缘自动对齐紧凑区域左边缘。右侧显示的是 macOS 原始菜单栏项目，
不存在覆盖其上的代理窗口，因此保持系统原生点击行为，也不申请辅助功能或录屏权限。图标不足、
未进入该区域时不做任何调整，也不会留下固定空白。
