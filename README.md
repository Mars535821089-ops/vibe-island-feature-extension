...
优先选择最接近小岛右边缘的原生插槽，再只调整这一个占位的长度，使其左边缘固定在
`X=1543`。可见小岛始终保持 `354 pt` 且绝对居中；占位宿主允许吸收最多 4 pt 左侧欠量
或一个受限的右侧插槽余量，避免用固定偏移在不同图标组合下左右漂移。
占位启用后，菜单项仍由 macOS 原生承载，不生成静态图标副本。Vibe Island 的透明宿主窗口
比可见的 354 pt 黑框更宽；当鼠标确实落在黑框外的原生菜单项上时，程序会在该次点击期间
临时让透明宿主退出命中区域，点击结束立即恢复，因此被移出的图标仍能打开其原菜单。恢复
同时受鼠标抬起、超时、正常退出、下次启动和运行期巡检五层保护；巡检只修复该机制使用的
精确离屏坐标，不改变正常窗口。点击黑框本身、没有原生菜单项的位置以及普通窗口均不处理。
程序不请求录屏权限；点击穿透依赖
...
因此登录重启、菜单图标增减和宽度变化都走同一套适配逻辑，而不是写死某次截图的坐标。
